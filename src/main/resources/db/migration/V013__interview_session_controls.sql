...
ALTER TABLE evidence
    ...
    ADD CONSTRAINT evidence_source_type_check CHECK (
        ...
        OR (source_type = 'interviewee_answer' AND question_id IS NOT NULL
            AND revises_evidence_id IS NULL)
...
ALTER TABLE interview_runtime_runs
    ...
    ADD CONSTRAINT interview_runtime_runs_source_check CHECK (
        (trigger IN ('session_start', 'resume')
            AND evidence_id IS NULL AND source_question_id IS NULL)
        OR (trigger = 'accepted_evidence'
            AND evidence_id IS NOT NULL AND source_question_id IS NULL)
        OR (trigger = 'clarification_request'
            AND evidence_id IS NULL AND source_question_id IS NOT NULL)
    ),
