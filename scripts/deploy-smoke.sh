...
curl --fail --silent --show-error "$PUBLIC_ORIGIN/actuator/health/readiness" >/dev/null

published=$("$container_cli" compose -f "$compose_file" ps --format '{{.Service}} {{.Publishers}}')
printf '%s\n' "$published" | grep '^ingress ' >/dev/null
if printf '%s\n' "$published" | grep -E '^(web|worker|migrate|egress) .*[0-9]+:' >/dev/null; then
  echo "A private service has a published host port" >&2
  exit 1
fi

"$container_cli" compose -f "$compose_file" exec -T web \
  wget --no-verbose --tries=1 --spider http://localhost:8080/actuator/health/readiness
"$container_cli" compose -f "$compose_file" exec -T worker \
  wget --no-verbose --tries=1 --spider http://localhost:8081/actuator/health/readiness
